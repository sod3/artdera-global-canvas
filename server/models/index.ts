...
userSchema.index({ emailNormalized: 1 }, { unique: true, collation: { locale: "en", strength: 2 } });
userSchema.index(
  { phoneNormalized: 1 },
...
export const storeSchema = new Schema(
  {
    ...
    province: { type: String, trim: true, maxlength: 100 },
    country: { type: String, trim: true, maxlength: 80, default: "Pakistan" },
    status: { type: String, enum: ["draft", "pending_review", "active", "suspended", "archived"], default: "draft", index: true },
    verificationStatus: verification,
    ...
    publicContactPreference: { type: String, enum: ["messages", "eligible_orders"], default: "messages" },
...
export const artworkSchema = new Schema(
  {
    ...
    status: { type: String, enum: ["draft", "pending_review", "published", "rejected", "reserved", "sold", "archived"], default: "draft", index: true },
    moderationStatus: { type: String, enum: ["not_submitted", "pending", "approved", "rejected"], default: "not_submitted", index: true },
...
artworkSchema.index({ title: "text", description: "text", tags: "text", category: "text", medium: "text" });
...
export const subscriptionPlanSchema = new Schema(
  {
    planId: { type: String, enum: ["free", "professional", "pro-plus", "gallery"], required: true, unique: true },
    name: requiredString,
    monthlyPrice: money,
...
export const subscriptionSchema = new Schema(
  {
    ...
    planId: { type: String, enum: ["free", "professional", "pro-plus", "gallery"], required: true, index: true },
    billingCycle: { type: String, enum: ["free", "monthly", "annual"], required: true },
    status: { type: String, enum: ["pending", "active", "payment_review", "payment_failed", "past_due", "cancelled", "expired", "suspended"], required: true, index: true },
    price: money,
    currency: { type: String, enum: ["PKR"], default: "PKR" },
    commissionRate: { type: Number, required: true, min: 0, max: 100 },
    listingLimit: { type: Number, min: 0, default: null },
...
export const paymentSchema = new Schema(
  {
    ...
    promotionId: { type: objectId, ref: "Promotion" },
    paymentType: { type: String, enum: ["subscription", "order", "promotion", "refund"], required: true },
    provider: { type: String, required: true },
    providerReference: { type: String, required: true, unique: true },
    amount: money,
    currency: { type: String, enum: ["PKR"], default: "PKR" },
    status: { type: String, enum: ["initiated", "pending", "processing", "successful", "failed", "cancelled", "refunded", "partially_refunded"], default: "initiated", index: true },
...
export const invoiceSchema = new Schema(
  {
    ...
    items: [{ description: requiredString, quantity: { type: Number, min: 1 }, unitPrice: { type: Number, min: 0 }, total: { type: Number, min: 0 } }],
    subtotal: money,
...
export const orderSchema = new Schema(
  {
    ...
    currency: { type: String, enum: ["PKR"], default: "PKR" },
    status: { type: String, enum: ["awaiting_payment", "paid", "seller_confirmed", "preparing", "ready_for_pickup", "shipped", "out_for_delivery", "delivered", "inspection_period", "completed", "return_requested", "returned", "refunded", "cancelled", "disputed"], default: "awaiting_payment", index: true },
    paymentStatus: { type: String, enum: ["unpaid", "pending", "paid", "failed", "refunded", "partially_refunded"], default: "unpaid", index: true },
  ...
  new Schema(
    {
      ...
      weight: { type: Number, min: 0 },
      dimensions: { width: Number, height: Number, depth: Number, unit: { type: String, enum: ["cm", "in"], default: "cm" } },
      ...
      status: { type: String, enum: ["estimate", "awaiting_pickup", "picked_up", "in_transit", "out_for_delivery", "delivered", "delayed", "damaged", "returned"], default: "estimate", index: true },
      pickedUpAt: Date,
      shippedAt: Date,
      deliveredAt: Date,
  ...
  new Schema(
    {
      ...
      netAmount: money,
      status: { type: String, enum: ["pending", "on_hold", "available", "processing", "completed", "failed", "reversed"], default: "pending", index: true },
      availableAt: Date,
      processedAt: Date,
...
export const conversationSchema = new Schema(
  {
    ...
    status: { type: String, enum: ["active", "archived", "blocked", "reported"], default: "active", index: true },
  ...
  new Schema(
    {
      ...
      type: { type: String, enum: ["text", "image", "document", "system", "offer", "video_request"], default: "text" },
      ...
      readAt: Date,
      moderationFlags: [{ type: String, enum: ["phone", "email", "whatsapp", "external_payment", "spam"] }],
    },
    timestamps,
  ),
);
  ...
  new Schema(
    {
      ...
      status: { type: String, enum: ["pending", "accepted", "rejected", "countered", "withdrawn", "expired"], default: "pending", index: true },
  ...
  new Schema(
    {
      ...
      message: { type: String, trim: true, maxlength: 1000 },
      status: { type: String, enum: ["requested", "accepted", "rejected", "alternate_suggested", "cancelled", "completed"], default: "requested" },
  ...
  new Schema(
    {
      ...
      storeId: { type: objectId, ref: "Store", required: true },
      artworkId: { type: objectId, ref: "Artwork" },
      promotionType: { type: String, enum: ["boost_3", "boost_7", "category_top", "featured_artist", "homepage", "social", "newsletter"], required: true },
      placement: requiredString,
      price: money,
      currency: { type: String, enum: ["PKR"], default: "PKR" },
      startAt: Date,
      endAt: Date,
      status: { type: String, enum: ["draft", "pending_payment", "pending_approval", "scheduled", "active", "completed", "rejected", "cancelled"], default: "draft", index: true },
      paymentId: { type: objectId, ref: "Payment" },
  ...
  new Schema(
    {
      ...
      status: { type: String, enum: ["pending", "approved", "suspended", "rejected"], default: "approved", index: true },
    },
    timestamps,
  ),
);
  ...
  new Schema(
    {
      ...
      status: { type: String, enum: ["open", "waiting", "resolved", "closed"], default: "open", index: true },
...
export const OneTimeTokenModel = modelFor(
  ...
  new Schema(
    {
      userId: { type: objectId, ref: "User", required: true, index: true },
      purpose: { type: String, enum: ["email_verification", "phone_verification", "password_reset"], required: true, index: true },
  ...
  new Schema(
    {
      ...
      items: [{ artworkId: { type: objectId, ref: "Artwork", required: true }, quantity: { type: Number, min: 1, max: 10, default: 1 }, priceSnapshot: { type: Number, min: 0 } }],
      expiresAt: Date,
    },
    timestamps,
  ),
  ...
  new Schema(
    {
      ...
      purpose: { type: String, enum: ["artwork", "profile", "cover", "message", "verification"], required: true },
...
export const TaxonomyModel = modelFor(
  ...
  new Schema(
    {
      type: { type: String, enum: ["category", "medium", "style", "city", "province"], required: true, index: true },
  ...
  new Schema(
    {
      ...
      type: { type: String, enum: ["store_view", "artwork_view", "save", "message", "offer", "video_request", "order", "promotion_click"], required: true, index: true },
...
export const PendingPlanSelectionModel = modelFor(
  ...
  new Schema(
    {
      tokenHash: { type: String, required: true, unique: true, select: false },
      planId: { type: String, enum: ["free", "professional", "pro-plus", "gallery"], required: true },
      billingCycle: { type: String, enum: ["free", "monthly", "annual"], required: true },
