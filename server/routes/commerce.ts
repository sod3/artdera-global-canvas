...
commerceRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (artworks.length !== cartItems.length)
      throw new ApiError(409, "CART_STALE", "One or more cart items are no longer available");
    ...
    for (const [storeId, group] of groups) {
      ...
      const shippingCost = Math.round(
        (rule?.baseCost ?? 1500) +
          totalWeight * (rule?.perKgCost ?? 250) +
          (fragile ? (rule?.fragileSurcharge ?? 500) : 0) +
          (framed ? (rule?.framingSurcharge ?? 300) : 0),
      );
      ...
      const intent = await paymentProvider().createPayment({
        amount: buyerTotal,
        currency: "PKR",
        idempotencyKey: `${req.auth!.user._id}_${storeId}_${randomUUID()}`,
      });
      prepared.push({
        store,
        subscription,
    ...
    return ok(
      ...
      {
        orders: results,
        estimateNotice: "Shipping amounts are ArtDera estimates, not confirmed courier quotes.",
      },
      "Checkout created",
      201,
    );
        ...
        if (!verified.successful) {
          ...
          for (const item of order.items) {
            await ArtworkModel.updateOne(
              { _id: item.artworkId, reservedBy: req.auth!.user._id },
              {
                $set: { status: "sold" },
                $inc: { soldCount: item.quantity, quantity: -item.quantity },
