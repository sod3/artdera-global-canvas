...
bootstrapRouter.get(
  ...
  asyncRoute(async (req, res) => {
    const [plans, stores, artworks, artistProfiles, galleryProfiles, taxonomies, collections, exhibitions] = await Promise.all([
      SubscriptionPlanModel.find({ isActive: true }).sort({ sortOrder: 1 }).lean(),
      StoreModel.find({ isPublished: true, status: "active" }).sort({ createdAt: -1 }).limit(100).lean(),
      ArtworkModel.find({ status: "published", moderationStatus: "approved" }).sort({ createdAt: -1 }).limit(100).lean(),
      ArtistProfileModel.find({ onboardingCompleted: true }).populate("userId", "fullName city province country avatarUrl").lean(),
      GalleryProfileModel.find({ onboardingCompleted: true }).populate("userId", "fullName city province country avatarUrl").lean(),
      ...
      ExhibitionModel.find({ isPublished: true, status: { $in: ["scheduled", "active"] } }).sort({ startAt: 1 }).lean(),
    ]);
    const publicData: Record<string, unknown> = {
      runtime: {
        ...
        promotionPlacements: Object.entries(PROMOTION_PRICES).map(([id, value]) => ({
          ...
          description: value.approval ? "Subject to ArtDera approval." : "Sponsored marketplace placement.",
        })),
      },
      ...
      creators: artistProfiles.map((profile: any) => ({
        ...
        slug: stores.find((store) => String(store.ownerId) === String(profile.userId?._id ?? profile.userId))?.slug ?? String(profile.displayName).toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      ...
      taxonomy: taxonomies.map((item) => ({ id: String(item._id), type: item.type, name: item.name, slug: item.slug, province: item.province })),
      collections: collections.map((item) => ({ id: String(item._id), name: item.name, slug: item.slug, description: item.description, artworkIds: item.artworkIds.map(String), coverImage: item.coverImageUrl })),
      exhibitions: exhibitions.map((item) => ({ id: String(item._id), galleryId: String(item.galleryId), name: item.name, slug: item.slug, coverImage: item.coverImageUrl, description: item.description, venue: item.venue, startDate: item.startAt?.toISOString(), endDate: item.endAt?.toISOString(), artistIds: item.artistIds.map(String), artworkIds: item.artworkIds.map(String), format: item.type === "online" ? "Online" : item.type === "physical" ? "Physical" : "Hybrid", published: item.isPublished })),
    };
    if (!req.auth) return ok(res, { ...publicData, private: null });
    const userId = req.auth.user._id;
    const role = req.auth.user.role;
    const seller = ["artist", "gallery", "gallery_staff"].includes(role);
    const [subscription, ownedStores, orders, conversations, notifications, payments, invoices, promotions, payouts, reviews, wishlist, follows] = await Promise.all([
      SubscriptionModel.findOne({ userId }).sort({ createdAt: -1 }).lean(),
      seller ? StoreModel.find({ ownerId: userId }).lean() : Promise.resolve([]),
      OrderModel.find(role === "buyer" ? { buyerId: userId } : seller ? { sellerId: userId } : {}).sort({ createdAt: -1 }).limit(200).lean(),
      ConversationModel.find({ participants: userId }).sort({ lastMessageAt: -1 }).limit(200).lean(),
      ...
      seller ? PayoutModel.find({ sellerId: userId }).sort({ createdAt: -1 }).lean() : Promise.resolve([]),
      ReviewModel.find(role === "buyer" ? { buyerId: userId } : seller ? { sellerId: userId } : {}).sort({ createdAt: -1 }).lean(),
    ...
    const privateArtworks = privateArtworkStoreIds.length
      ? await ArtworkModel.find({ storeId: { $in: privateArtworkStoreIds } }).sort({ createdAt: -1 }).lean()
      : [];
    const conversationIds = conversations.map((conversation) => conversation._id);
    const messages = conversationIds.length
      ? await MessageModel.find({ conversationId: { $in: conversationIds } }).sort({ createdAt: 1 }).limit(1000).lean()
      : [];
    return ok(res, {
      ...publicData,
      private: {
        ...
        conversations: conversations.map((item) => ({ ...item, id: String(item._id), participantIds: item.participants.map(String), _id: undefined })),
        messages: messages.map((item) => ({ id: String(item._id), conversationId: String(item.conversationId), senderId: String(item.senderId), body: item.text, createdAt: item.createdAt.toISOString(), read: item.isRead, moderationFlags: item.moderationFlags })),
        notifications: notifications.map((item) => ({ id: String(item._id), userId: String(item.userId), type: item.type, title: item.title, body: item.message, read: item.isRead, createdAt: item.createdAt.toISOString(), href: item.link })),
