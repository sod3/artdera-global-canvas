...
const artworkInput = z
  .object({
    ...
    orientation: z.enum(["portrait", "landscape", "square", "Portrait", "Landscape", "Square"]).default("portrait"),
    ...
    images: z.array(z.object({ id: z.string().optional(), url: z.string().min(1).max(1500), alt: z.string().max(180).default(""), isPrimary: z.boolean().default(false) }).strip()).max(20).default([]),
    ...
    status: z.enum(["draft", "pending_review", "published", "rejected", "reserved", "sold", "archived", "Draft", "Pending Review", "Published", "Rejected", "Reserved", "Sold", "Archived"]).default("draft"),
...
function normalizeArtwork(input: z.infer<typeof artworkInput>) {
  ...
  const artworkType = input.artworkType ?? (input.kind === "Print" ? "print" : input.kind === "Limited Edition" ? "limited_edition" : "original");
  ...
  return {
    ...
    images: input.images.map((image) => ({ url: image.url, alt: sanitizeText(image.alt, 180), isPrimary: image.isPrimary })),
    ...
    status,
    moderationStatus: status === "pending_review" ? "pending" : status === "published" ? "approved" : "not_submitted",
...
artworksRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (req.query.verified === "true") {
      const stores = await StoreModel.find({ verificationStatus: "approved", isPublished: true }).select("_id").lean();
    ...
    const now = new Date();
    const activePromotions = await PromotionModel.find({ status: "active", startAt: { $lte: now }, endAt: { $gt: now }, artworkId: { $exists: true } })
    ...
    const organic = await ArtworkModel.find(organicFilter).sort(sort).skip(skipOrganic).limit(limit).populate("artistId", "fullName").populate("storeId", "name").lean();
    const sponsored = sponsoredIds.length
      ? await ArtworkModel.find({ ...filter, _id: { $in: sponsoredIds } }).sort({ _id: 1 }).limit(Math.ceil(limit / 5)).populate("artistId", "fullName").populate("storeId", "name").lean()
      : [];
...
artworksRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const items = await ArtworkModel.find({ storeId: { $in: stores.map((store) => store._id) } }).sort({ createdAt: -1 }).lean();
...
artworksRouter.get(
  "/slug/:slug",
  asyncRoute(async (req, res) => {
    const item = await ArtworkModel.findOne({ slug: slug.parse(req.params.slug), status: "published", moderationStatus: "approved" })
...
artworksRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!store) throw new ApiError(404, "STORE_NOT_FOUND", "Create your store before adding artwork");
    const normalized = normalizeArtwork(input);
    if (normalized.internationalShipping) await requirePermission(req.auth!.user._id, "international-tools");
    ...
    try {
      const item = await ArtworkModel.create({
        ...
        slug: normalized.slug ?? `${input.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "").slice(0, 80)}-${Date.now().toString(36)}`,
      });
      if (normalized.status === "pending_review")
        await notify(req.auth!.user._id, "artwork_submitted", "Artwork submitted", `${item.title} is awaiting moderation.`, "/artist/dashboard/artworks");
...
artworksRouter.patch(
  ...
  requireRole("artist", "gallery"),
  asyncRoute(async (req, res) => {
    const input = z.object({ ids: z.array(z.string()).min(1).max(200), action: z.enum(["submit", "archive", "restore", "delete"]) }).strict().parse(req.body);
    const stores = await StoreModel.find({ ownerId: req.auth!.user._id }).select("_id").lean();
    const items = await ArtworkModel.find({ _id: { $in: input.ids }, storeId: { $in: stores.map((store) => store._id) } });
    if (items.length !== new Set(input.ids).size) throw new ApiError(403, "FORBIDDEN", "One or more artworks cannot be changed");
    if (input.action === "delete") {
      if (items.some((item) => !["draft", "archived", "rejected"].includes(item.status)))
        throw new ApiError(409, "ARTWORK_DELETE_NOT_ALLOWED", "Only drafts, rejected, or archived artworks can be deleted");
    ...
    for (const item of items) {
      ...
      const next = input.action === "submit" ? "pending_review" : input.action === "archive" ? "archived" : "draft";
    ...
    return ok(res, items.map((item) => publicArtwork(item.toObject())), "Artworks updated");
  }),
);
...
artworksRouter.patch(
  ...
  asyncRoute(async (req, res) => {
    ...
    const stores = await StoreModel.find({ ownerId: req.auth!.user._id }).select("_id").lean();
    const item = await ArtworkModel.findOne({ _id: req.params.id, storeId: { $in: stores.map((store) => store._id) } });
    if (!item) throw new ApiError(404, "ARTWORK_NOT_FOUND", "Artwork not found");
...
artworksRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!["draft", "archived", "rejected"].includes(item.status))
      throw new ApiError(409, "ARTWORK_DELETE_NOT_ALLOWED", "Archive this artwork before deleting it");
