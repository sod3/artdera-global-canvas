...
function Checkout() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setCompleted({
      numbers: orders.map((entry) => entry.order.orderNumber),
      paid: DEMO_PAYMENT_MODE,
    });
    setSubmitting(false);
  ...
  if (user.role !== "buyer")
    return (
      <div className="container-editorial py-20 text-center">
        <h1 className="font-display text-5xl">Buyer checkout only.</h1>
      </div>
    );
  if (completed)
    return (
      <div className="container-editorial flex min-h-[70vh] items-center justify-center py-14">
        <div className="max-w-2xl rounded-3xl bg-[var(--ink)] p-8 text-center text-[var(--ivory)] md:p-12">
          ...
          <div className="eyebrow mt-7 !text-white/45">
            {completed.paid ? "Payment confirmed" : "Payment initiated"}
          </div>
          ...
          <p className="mx-auto mt-4 max-w-lg text-sm leading-relaxed text-white/62">
            {completed.paid
              ? "The development payment adapter confirmed your payment. "
              : "Complete payment using the configured provider instructions. "}
    ...
    <form onSubmit={submit} className="container-editorial py-12 lg:py-16">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed">
                No external charge is made. Payment outcomes are recorded through the server-side
                demo adapter and this mode is rejected in production.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <main className="space-y-6">
          ...
          <CheckoutSection title="Delivery address">
            <div className="grid gap-4 sm:grid-cols-2">
              <CheckoutField label="Address line" wide>
                <input required name="line1" autoComplete="address-line1" className="art-field" />
              </CheckoutField>
              ...
              <CheckoutField label="Postal code">
                <input name="postalCode" autoComplete="postal-code" className="art-field" />
              </CheckoutField>
          ...
          <CheckoutSection title="Payment method">
            <div className="grid gap-3 sm:grid-cols-2">
              {methods.map(([Icon, label, value, description]) => (
                <button
                  type="button"
                  key={value}
                  onClick={() => setMethod(value)}
                  className={`flex gap-3 rounded-xl border p-4 text-left ${method === value ? "border-[var(--oxblood)] bg-[var(--ivory)]" : "border-[var(--color-border)]"}`}
                >
            ...
            <div className="mt-4 flex gap-3 rounded-xl bg-[var(--ivory)] p-4 text-xs leading-relaxed text-muted-foreground">
              ...
              Sensitive payment credentials are handled by the payment provider, never stored in
              ArtDera forms.
            </div>
          </CheckoutSection>
        </main>
        <aside className="h-fit rounded-2xl border border-[var(--color-border)] bg-[var(--porcelain)] p-6 lg:sticky lg:top-28">
          ...
          {loading ? (
            <p className="mt-5 text-sm text-muted-foreground">Loading your cart…</p>
          ) : (
          ...
          <dl className="mt-6 divide-y divide-[var(--color-border)] text-sm">
            ...
            <div className="flex justify-between py-3">
              <dt>Shipping, packaging & fees</dt>
              <dd>Calculated securely</dd>
            </div>
          </dl>
          <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
            The final total is calculated from live shipping rules when the order is submitted and
            recorded on its invoice.
          </p>
          ...
          <button
            disabled={!agreed || !items.length || unavailable || loading || submitting}
