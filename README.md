...
Requirements: Node.js 20+, npm, and a MongoDB Atlas cluster or MongoDB replica set (transactions are used for checkout and paid subscription activation).
...
Uploads are streamed through Multer, checked by size, extension, MIME type, and file signature, then stored through an adapter. Verification documents are private. MongoDB stores file references, not Base64 image bodies.
...
- Uploads: `server/routes/uploads.ts` currently supports local public/private storage. Replace the adapter with object storage and signed private reads for production.
...
- Use private object storage for identity documents and production artwork uploads.
